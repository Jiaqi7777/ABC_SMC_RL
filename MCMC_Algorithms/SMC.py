...
class SMC:
    ...
    def __init__(self, model, Model, min_ess=0.2, num_samples=None, initial_params=None, params_dim=None):
    ...
    def update(self, obs, alpha, smc_samples):
        epsilon = self.SMC_Model.abclikelihood.epsilon
        epsilon_0 = self.find_epsilon_0(alpha=alpha, smc_samples=smc_samples, generate_new_weights_fn=generate_new_weights_0, epsilon_0=epsilon)
        print('epsilon_0 ==========', '\n', epsilon_0)
        # epsilon_0 = epsilon * 2
        ...
        while epsilon_0 > epsilon:
            print('Solving...')
            epsilon_0 = max(epsilon, self.find_epsilon_0(alpha=alpha, smc_samples=smc_samples, generate_new_weights_fn=generate_new_weights), epsilon_0=epsilon_0)
            # epsilon_0 *= 0.9
    ...
    def resample(self):
        ...
        smc_samples = torch.tensor([])
        for j in range(self.n_particle):
            posterior_samples, accept_probs, mcmc, kernel, logdensities, proposed_logdensities, = MCMC_update(Model=self.SMC_Model, posterior_samples=[paras[j]], env=env, training_steps_with_burnin=training_steps_with_burnin, training_steps=training_steps, USE_AUTOGRAD=USE_AUTOGRAD)
            # print(posterior_samples.shape)
            smc_samples = torch.cat((smc_samples, (posterior_samples[-1]).unsqueeze(0)))
    ...
    def find_epsilon_0(self, alpha, smc_samples, generate_new_weights_fn, epsilon_0):
        # Use optimization to find epsilon_0 that satisfies the ESS condition
        result = bisect(ESS_Matching, a=epsilon_0+1e-5, b=epsilon_0*1.5, xtol=1e-2, maxiter=2000, args=(alpha, self.SMC_Model, self._weights, smc_samples, generate_new_weights_fn, epsilon_0))
        return result
        # if result.success:
        #     return result.x[0]
        # else:
...
def generate_new_weights_0(epsilon, weights, Model, smc_samples, epsilon_0):
    new_weights = torch.tensor([w * torch.exp(Model.llh(parameter=p, new_epsilon=epsilon)[0]) for w, p in zip(weights, smc_samples)])
    return new_weights / torch.sum(new_weights)

def generate_new_weights(epsilon, weights, Model, smc_samples, epsilon_0):
    new_weights = torch.tensor([w * torch.exp(Model.llh(parameter=p, new_epsilon=epsilon)[0] - Model.llh(parameter=p, new_epsilon=epsilon_0)[0]) for w, p in zip(weights, smc_samples)])
    return new_weights / torch.sum(new_weights)
...
def SMC_update(Model, obs, model, alpha, smc_samples):
    smc = SMC(model=model, Model=Model, initial_params=smc_samples)
    smc.update_history(smc_samples, model._weights)
            ...
            for h in tqdm(range(HORIZON)):
                ...
                if done or ( h + 1)  % FROZEN_T == 0:
                    ...
                    if new_data_flag and e > 0:
                        ...
                        smc = SMC_update(Model=Model, obs=obs, model=model, alpha=alpha, smc_samples=smc_samples)
                    if TRANSFORM:
                    ...
                    # display_results(posterior_samples=smc_samples, model=model, accept_probs=accept_probs, logdensities=logdensities, proposed_logdensities=proposed_logdensities)
                if done:
