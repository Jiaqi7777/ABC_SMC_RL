...
def exact_solution(obs, mod, epsilon, sigma, state_q):
    ...
    goal_states = list(set([next_states[i] for i in range(len(next_states)) if done[i] is True]))
        ...
        for i in range(n):
            for j in range(theta_len):
                ...
                if state1 not in goal_states:
                    ...
                    if j == nu(mod=mod, state=state1, action=ell[ell_idx]):
                        B_ell[i,j] -= 1
    ...
    for idx in range(len(ell_list)):
        ...
        pr_vec[idx] = multivariate_normal.pdf(obs["rewards"],mean=np.zeros(n),cov=pr_cov)
